...
public class DAnswerFilter implements PixelFilter, Interactive, Drawable {
    ...
    private int xODist, startingX, startingY;
    ...
    public DAnswerFilter() {
        ...
        System.out.println("Supply the number of rows and columns and the number of bubbles per question, separated by a space:");
        String input = kb.nextLine();

        rows = Integer.parseInt(input.split(" ")[0]);
        columns = Integer.parseInt(input.split(" ")[1]);
    ...
    private void boxImage(short[][][] cChannels) {
        for (int c = startingX / 2; c < (rows * xODist / 2); c += xODist / 2) {
            for (int r = startingY / 2; r < startingY / 2 + columns * (dist) / 2; r += dist / 2) {
    ...
    private void makeDBox(short[][][] iChannels, int row, int col) {
        ...
        for (int c = 0; c < boxColor.length; c++) {
            for (int i = row; i <= row + dist / 2; i++) {
                ...
                iChannels[c][i][col + xODist / 2] = (short) boxColor[c];
            }

            for (int i = col; i <= col + xODist / 2; i++) {
                iChannels[c][row][i] = (short) boxColor[c];
                iChannels[c][row + dist / 2][i] = (short) boxColor[c];
            }
        }
    }
    ...
    private void saveToCSV() {
        try {
            ...
            out.println(rows + " " + columns + " " + startingX + " " + startingY + " " + xODist + " " + answers);
